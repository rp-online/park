...
(() => {
  ...
  function HookForBidReceived(divId, adapterId) {
    console.log('HookForBidReceived: ', adapterId.bidderCode);
    if (adapterId.bidderCode === 'justpremium') {
      skystopp = 'true';
      window.googletag.cmd.push(() => {
        window.googletag.pubads().setTargeting('skystopp', skystopp);
          ...
          window.park.resourceLoader.script(pubmaticUrl).then(() => {
            window.apstag.fetchBids({
              slots: slots.reduce((accumulator, slot) => {
                if (slot.adSlotName && slot.size && slot.size !== [] && !(slot.size.length === 1 && slot.size.includes('fluid'))) {
                  accumulator.push({
  ...
  function loadAndInit() {
    if (window.requestIdleCallback) {
      window.requestIdleCallback(loadAndInitProcess, { timeout: 5000 });
      return;
    }
